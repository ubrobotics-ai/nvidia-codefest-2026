...
def write_md(res, out, chance, think=None, agr=None, mcn=None, trt_rows=None):
    ...
    if TRT_LBL in res and BF_LBL in res and ct_rows_for_k:
        ...
        A("*The left prior.* left_right is balanced 250/250, and the model answers \"left\" 72.4% of the time:")
        A("**87.2% correct when the answer is left, 42.4% when it is right** — below chance on half the set. It")
        A("survives quantisation almost unchanged (70.0%, 84.0%/44.0% on the TensorRT arm), so it is a property of")
        A("the model, not of the 4-bit path. That is a decision-threshold problem sitting on 500 items, and a")
        A("balanced-prior correction is the cheapest accuracy left on the table anywhere in this report.\n")
