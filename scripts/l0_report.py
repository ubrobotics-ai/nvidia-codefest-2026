...
def write_md(res, out, chance, think=None, agr=None, mcn=None, trt_rows=None):
    ...
    if TRT_LBL in res and BF_LBL in res and ct_rows_for_k:
        ...
        A("**Why not output format, which an earlier draft of this report recommended.** That recommendation")
        A("predates the tolerant parser and the Cosmos-sees / Gemma-decides split. Under the split, Cosmos never")
        A("emits the schema and no longer needs to — the contract lives in a deterministic converter — while")
        A("Gemma already writes it at 0.958 validity. Training Cosmos to emit a schema it is not asked to emit")
        A("buys nothing. If format SFT is argued for again, it needs a reason the parser does not already cover.\n")
