...
def write_md(res, out, chance, think=None, agr=None, mcn=None, trt_rows=None):
    ...
    if TRT_LBL in res and BF_LBL in res and ct_rows_for_k:
        ...
        A("- **Distance k is fitted per engine build, provisionally 1.21.** Not a model constant.")
        A("- **Thinking on for grounding, off for closed spatial queries.** Per-request flag; no global choice.")
        ...
        A("(discordant pairs, not accuracies; held out, bf16 174 vs TensorRT 167 of 449). left_right")
        ...
        A("these items (in-sample, own k, over the 449 usable): the INT4 answers score")
        A(f"{wk['pct_right']:.2f}% ({wk['right']}/449) under their own constant and {wk['pct_wrong']:.2f}%")
        A(f"({wk['wrong']}/449) under the bf16 one —")
        ...
        A("**The Friday SFT decision.** Not output format, and not precision. Two candidates beat both.\n")
        A("*The left prior — measured, and it is not the cheap win an earlier draft of this report claimed.*")
        A("left_right is balanced 250/250 and the model answers \"left\" 72.4% of the time: **87.2% correct when")
        A("the answer is left, 42.4% when it is right**, below chance on half the set, and it survives")
        A("quantisation nearly unchanged. The obvious fix is order-debiasing — ask the mirrored question about")
        A("the same image and average — so it was run on all 500 items against the TensorRT engine:\n")
        A("| | n = 500 |")
        A("|---|---|")
        ...
        A("")
        A("**Order-debiasing makes it worse, and cannot help.** On two thirds of items the model returns the same")
        A("word for a question and its mirror, so there is no latent spatial judgement on those items to recover;")
        A("averaging can at best keep the 121 it gets right when consistent and guess the rest, a ceiling near 57%")
        A("— below the 64% it already scores. The prior is not a mis-set threshold on a working comparison. It is")
        A("the absence of the comparison on most items, partly masked by a bias that happens to pay.\n")
        A("What survives: the model is meaningfully better when it *is* order-consistent (70.35% against 60.7% on")
        A("the rest), so order-consistency is a usable **confidence signal** at 2x inference cost — worth having")
        A("on a rover that can decline to answer. And a logit-bias fix cannot be evaluated from these outputs at")
        A("all, because the harness saves decoded text, not scores. Any SFT case here has to be argued on the")
        A("65.6% order-invariance, not on the 72/28 answer split.\n")
        A("*Open-vocabulary precision.* L1 measures 0.529 precision on the case where Cosmos is the only proposer")
        A("and nothing adjudicates. That is a perception problem, and it is the one that decides whether a box")
        A("means \"go and look\" or \"found\".\n")
        A("**Why not output format.** An earlier draft of this report recommended it. That predates two things:")
        A("the tolerant parser, which recovers the boxes (precision 0.529, recall 0.750) without the model ever")
        A("emitting valid JSON, and L1's 2026-09-11 decision that Cosmos3-Edge takes **both** jobs, perception and")
        A("the operator interface, with geometry deterministic in code. Under that architecture nothing asks the")
        A("model for a contract. L1 also tried six contract rewrites — flat fixed-arity, single slot, line-per-")
        A("entity, worked example, ROS 2 `Detection2DArray` — and **every one scored 0/12 detections** while")
        A("improving format. Training the model toward a format it is not asked to produce would buy the thing")
        A("that already works and cost the thing that does not.\n")
        A("L1's own SFT candidates are **tool-calling** (0/21, but measured in August on a different serving stack")
        A("with a bare schema, and unmeasured under the prompt that moved commands from 3/14 to 14/15) and the")
        A("missing **UNKNOWN** path — `\"Sing me a song\"` returns `BACK` rather than a refusal. The left prior above")
        A("is a third, and the only one this report contributes.\n")
    ...
    A("| arm | [0,4) m | [4,8) | [8,12) | [12,+) | global | spread |")
    A("|---|---|---|---|---|---|---|")
    ...
    A("(167 -> 183 of 486), **+0.00** on bf16 and **-1.44** on NF4. So AWQ's under-read is range-dependent and")
    ...
    A("Jetson also consumes. Engines are per-SM and not shared: the SM103 plan measured here is 884 MB against")
    A("the Orin SM87 plan's 838.9 MiB from the same ONNX.\n")
